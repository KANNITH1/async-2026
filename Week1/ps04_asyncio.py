...
async def make_coffee(customer_name):
    # 1. ดู Process ID และ Thread ID (ซึ่งจะพบว่าเหมือนกันทุกครั้ง)
    pid = os.getpid()
...
async def main():
    ...
    print(f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] === เริ่มระบบจำลองผู้ทำกาแฟแบบ asyncio ===")
    start_time = time()
    ...
    for customer in queue:
        ...
        # แปลง Coroutine ให้เป็น Task เพื่อให้ Event Loop บริหาร และสั่งซื้อได้
