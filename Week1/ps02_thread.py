...
def make_coffee(customer_name):
    ...
    sum(i * i for i in range(1000000) ) # จำลองงานคำนวณ (CPU-bound) เล็กนอย และรอ 5 วินาที
    sleep(5) # บล็อกการทำงานของ Thread นีไว้ 5 วินาทีเต็มๆ
...
def main():
    ...
    print (f"{ctime()} | [Main PID: {main_pid}] [Main TID: {main_tid}] mmm เริ่มระบบจำลองตู้กาแฟแบบ Multi-Thread mm=")
    start_time = time()
    ...
    for customer in queue:
        # เราสามารถดังชื่อ Thread ผ่านพารามิเตอร์ name= ได้เพื่อให้ไล้โค้ดง่ายขึ้น
    ...
    # ใช่ psutil ดึงค่าการกิน RAM
    process = psutil.Process(os.getpid())
    mem_mb = process.memory_info().rss / (1024 * 1024)

    print(f"[w7wum Multi-Thread]")
    ...
    print(f"ทรัพยากร Memory (RAM) ที่ใช่: {mem_mb: .2f} MB")
